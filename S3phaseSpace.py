...
outerBarX_7525 = np.nan*np.ones((len(thetasMode2),len(thetasMode1)))
...
for tt in range(len(thetasMode1)):
   ...
   for yy in range(len(thetasMode2)):
      ...
      if xFRFbar is not None:
         if len(xFRFbar) > 1:
            outerBarX_7525[yy,tt] = np.max(xFRFbar)
         else:
            innerOuterBarX_7525[yy,tt] = xFRFbar



outerBarX_2575 = np.nan*np.ones((len(thetasMode2),len(thetasMode1)))
...
for tt in range(len(thetasMode1)):
   ...
   for yy in range(len(thetasMode2)):
      ...
      if xFRFbar is not None:
         if len(xFRFbar) > 1:
            outerBarX_2575[yy,tt] = np.max(xFRFbar)
         else:
            innerOuterBarX_2575[yy,tt] = xFRFbar




outerBarX_7575 = np.nan*np.ones((len(thetasMode2),len(thetasMode1)))
innerOuterBarX_7575 = np.nan*np.ones((len(thetasMode2),len(thetasMode1)))
...
for tt in range(len(thetasMode1)):
   ...
   for yy in range(len(thetasMode2)):
      ...
      if xFRFbar is not None:
         if len(xFRFbar) > 1:
            outerBarX_7575[yy,tt] = np.max(xFRFbar)
         else:
            innerOuterBarX_7575[yy,tt] = xFRFbar


outerBarX_8585 = np.nan*np.ones((len(thetasMode2),len(thetasMode1)))
innerOuterBarX_8585 = np.nan*np.ones((len(thetasMode2),len(thetasMode1)))

for tt in range(len(thetasMode1)):
   eofPred = SS[:, 0] * np.nanpercentile(RS[:,0],85) * np.cos(thetasMode1[tt] * np.pi / 180 - thetaS[:, 0])
   for yy in range(len(thetasMode2)):
      eofPred2 = SS[:, 1] * np.nanpercentile(RS[:,1],85) * np.cos(thetasMode2[yy] * np.pi / 180 - thetaS[:, 1])
      combined = np.real(np.mean(alllinesS,axis=0)+eofPred+eofPred2)
      xFRFbar, xFRFtrough = sandbarUtils.findSandBarAndTrough1D(xinterpS, combined, plotFname=None, smoothLengthScale=5, profileTrend=np.mean(alllinesS,axis=0))
      if xFRFbar is not None:
         if len(xFRFbar) > 1:
            outerBarX_8585[yy,tt] = np.max(xFRFbar)
         else:
            innerOuterBarX_8585[yy,tt] = xFRFbar

outerBarX_2525 = np.nan * np.ones((len(thetasMode2), len(thetasMode1)))
...
for tt in range(len(thetasMode1)):
    ...
    for yy in range(len(thetasMode2)):
        ...
        xFRFbar, xFRFtrough = sandbarUtils.findSandBarAndTrough1D(xinterpS, combined, plotFname=None,
                                                                  smoothLengthScale=5,
                                                                  profileTrend=np.mean(alllinesS, axis=0))
        if xFRFbar is not None:
            if len(xFRFbar) > 1:
                outerBarX_2525[yy, tt] = np.max(xFRFbar)
            else:
                innerOuterBarX_2525[yy, tt] = xFRFbar


outerBarX_8550 = np.nan * np.ones((len(thetasMode2), len(thetasMode1)))
innerOuterBarX_8550 = np.nan * np.ones((len(thetasMode2), len(thetasMode1)))

for tt in range(len(thetasMode1)):
    eofPred = SS[:, 0] * np.nanpercentile(RS[:, 0], 85) * np.cos(thetasMode1[tt] * np.pi / 180 - thetaS[:, 0])
    for yy in range(len(thetasMode2)):
        eofPred2 = SS[:, 1] * np.nanpercentile(RS[:, 1], 50) * np.cos(thetasMode2[yy] * np.pi / 180 - thetaS[:, 1])
        combined = np.real(np.mean(alllinesS, axis=0) + eofPred + eofPred2)
        xFRFbar, xFRFtrough = sandbarUtils.findSandBarAndTrough1D(xinterpS, combined, plotFname=None,
                                                                  smoothLengthScale=5,
                                                                  profileTrend=np.mean(alllinesS, axis=0))
        if xFRFbar is not None:
            if len(xFRFbar) > 1:
                outerBarX_8550[yy, tt] = np.max(xFRFbar)
            else:
                innerOuterBarX_8550[yy, tt] = xFRFbar




outerBarX_9066 = np.nan * np.ones((len(thetasMode2), len(thetasMode1)))
innerOuterBarX_9066 = np.nan * np.ones((len(thetasMode2), len(thetasMode1)))

for tt in range(len(thetasMode1)):
    eofPred = SS[:, 0] * np.nanpercentile(RS[:, 0], 90) * np.cos(thetasMode1[tt] * np.pi / 180 - thetaS[:, 0])
    for yy in range(len(thetasMode2)):
        eofPred2 = SS[:, 1] * np.nanpercentile(RS[:, 1], 66) * np.cos(thetasMode2[yy] * np.pi / 180 - thetaS[:, 1])
        combined = np.real(np.mean(alllinesS, axis=0) + eofPred + eofPred2)
        xFRFbar, xFRFtrough = sandbarUtils.findSandBarAndTrough1D(xinterpS, combined, plotFname=None,
                                                                  smoothLengthScale=5,
                                                                  profileTrend=np.mean(alllinesS, axis=0))
        if xFRFbar is not None:
            if len(xFRFbar) > 1:
                outerBarX_9066[yy, tt] = np.max(xFRFbar)
            else:
                innerOuterBarX_9066[yy, tt] = xFRFbar
...
doublebar_7525 = copy.deepcopy(outerBarX_7525)
doubleIndex_7525 = np.where((doublebar_7525 > 0))
doublebar_7525[doubleIndex_7525] = 0.5
singlebar_7525 = copy.deepcopy(innerOuterBarX_7525)
singleIndex_7525 = np.where((singlebar_7525 > 0))
doublebar_7525[singleIndex_7525] = 0.25


doublebar_2575 = copy.deepcopy(outerBarX_2575)
doubleIndex_2575 = np.where((doublebar_2575 > 0))
doublebar_2575[doubleIndex_2575] = 0.5
singlebar_2575 = copy.deepcopy(innerOuterBarX_2575)
singleIndex_2575 = np.where((singlebar_2575 > 0))
doublebar_2575[singleIndex_2575] = 0.25


doublebar_7575 = copy.deepcopy(outerBarX_7575)
doubleIndex_7575 = np.where((doublebar_7575 > 0))
doublebar_7575[doubleIndex_7575] = 0.5
singlebar_7575 = copy.deepcopy(innerOuterBarX_7575)
singleIndex_7575 = np.where((singlebar_7575 > 0))
doublebar_7575[singleIndex_7575] = 0.25

doublebar_8585 = copy.deepcopy(outerBarX_8585)
doubleIndex_8585 = np.where((doublebar_8585 > 0))
doublebar_8585[doubleIndex_8585] = 0.5
singlebar_8585 = copy.deepcopy(innerOuterBarX_8585)
singleIndex_8585 = np.where((singlebar_8585 > 0))
doublebar_8585[singleIndex_8585] = 0.25

doublebar_2525 = copy.deepcopy(outerBarX_2525)
doubleIndex_2525 = np.where((doublebar_2525 > 0))
doublebar_2525[doubleIndex_2525] = 0.5
singlebar_2525 = copy.deepcopy(innerOuterBarX_2525)
singleIndex_2525 = np.where((singlebar_2525 > 0))
doublebar_2525[singleIndex_2525] = 0.25

doublebar_8550 = copy.deepcopy(outerBarX_8550)
doubleIndex_8550 = np.where((doublebar_8550 > 0))
doublebar_8550[doubleIndex_8550] = 0.5
singlebar_8550 = copy.deepcopy(innerOuterBarX_8550)
singleIndex_8550 = np.where((singlebar_8550 > 0))
doublebar_8550[singleIndex_8550] = 0.25

doublebar_9066 = copy.deepcopy(outerBarX_9066)
doubleIndex_9066 = np.where((doublebar_9066 > 0))
doublebar_9066[doubleIndex_9066] = 0.5
singlebar_9066 = copy.deepcopy(innerOuterBarX_9066)
singleIndex_9066 = np.where((singlebar_9066 > 0))
doublebar_9066[singleIndex_9066] = 0.25
...
ax100 = plt.subplot2grid((2,3),(0,1),rowspan=1,colspan=1)
con1 = ax100.pcolor(meshTheta1,meshTheta2,doublebar_7525,vmin=0,vmax=1,cmap='Greys')
coloredlines = cm.rainbow(np.linspace(0,1,len(lowWaves)))
lineSlopes = []
lineSlopeTimes = []
for ff in range(len(lowWaves)):
...
ax100 = plt.subplot2grid((2,3),(0,2),rowspan=1,colspan=1)
con1 = ax100.pcolor(meshTheta1,meshTheta2,doublebar_2575,vmin=0,vmax=1,cmap='Greys')
coloredlines = cm.rainbow(np.linspace(0,1,len(lowWaves)))
lineSlopes = []
lineSlopeTimes = []
for ff in range(len(lowWaves)):
    si = lowWaves[ff][0]
    ei = lowWaves[ff][1]
    indexedTime = time3[si-2:ei+0]

    indexedOrdinalTime = [i.toordinal() for i in indexedTime]
    indexedTimeMonth = [i.month for i in indexedTime]
    mode1prop = mode1Phase3WA[si-2:ei+0]
    mode2prop = mode2Phase3WA[si-2:ei+0]
    # mode1prop = mode1Phase3WA[si-1:ei+1]
    # mode2prop = mode2Phase3WA[si-1:ei+1]
    mode1propWrap = np.unwrap((mode1prop + np.pi) % (2 * np.pi) - np.pi)
    mode2propWrap = np.unwrap((mode2prop + np.pi) % (2 * np.pi) - np.pi)
    x1 = np.sort(mode1propWrap*180/np.pi)
    y1 = mode2propWrap*180/np.pi
    mags1 = mode1Mag[si:ei+2+0]
    mags2 = mode2Mag[si:ei+2+0]
...
ax100 = plt.subplot2grid((2,3),(1,0),rowspan=1,colspan=1)
con1 = ax100.pcolor(meshTheta1,meshTheta2,doublebar_8550,vmin=0,vmax=1,cmap='Greys')
coloredlines = cm.rainbow(np.linspace(0,1,len(lowWaves)))
lineSlopes = []
lineSlopeTimes = []
for ff in range(len(lowWaves)):
...
ax100 = plt.subplot2grid((2,3),(1,1),rowspan=1,colspan=1)
con1 = ax100.pcolor(meshTheta1,meshTheta2,doublebar_8585,vmin=0,vmax=1,cmap='Greys')
coloredlines = cm.rainbow(np.linspace(0,1,len(lowWaves)))
lineSlopes = []
lineSlopeTimes = []
for ff in range(len(lowWaves)):
...
ax100 = plt.subplot2grid((2,3),(1,2),rowspan=1,colspan=1)
con1 = ax100.pcolor(meshTheta1,meshTheta2,doublebar_2525,vmin=0,vmax=1,cmap='Greys')
coloredlines = cm.rainbow(np.linspace(0,1,len(lowWaves)))
lineSlopes = []
lineSlopeTimes = []
for ff in range(len(lowWaves)):
...
ax100 = plt.subplot2grid((2,3),(1,0),rowspan=1,colspan=1)
con1 = ax100.pcolor(meshTheta1,meshTheta2,doublebar_9066,vmin=0,vmax=1,cmap='Greys')
coloredlines = cm.rainbow(np.linspace(0,1,len(lowWaves)))
lineSlopes = []
lineSlopeTimes = []
for ff in range(len(lowWaves)):
